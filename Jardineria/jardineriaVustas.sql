-- Base de datos: Jardinería: SQL AÑADIDO EN LA CARPETA CON EL CODIGO DE LA DB
-- 1. Escriba una vista que se llame listado_pagos_clientesque muestre un listado donde aparezcan todos los
...
select concat_ws(" ", C.nombre_contacto, C.apellido_contacto), C.telefono, C.ciudad, C.pais, P.fecha_pago, P.total, P.id_transaccion 
...
-- create view listado_pagos_clientes as 
select concat_ws(" ",C.nombre_contacto, C.apellido_contacto), C.telefono, C.ciudad, C.pais, P.codigo_pedido,
 P.fecha_pedido, P.fecha_esperada, P.fecha_entrega, sum(DP.cantidad * DP.precio_unidad) as total_pedido
...
-- create view listado_pagos_clientes as 
select concat_ws(" ",C.nombre_contacto, C.apellido_contacto), C.telefono, C.ciudad, C.pais, P.fecha_pago, P.total, P.id_transaccion 
from cliente C left join pago P on P.codigo_cliente=C.codigo_cliente where C.ciudad = 'Madrid' and P.total IS NOT NULL;
...
select nombre, max(total_pedido) as maximo, min(total_pedido) as minimo from listado_pedidos_clientes group by nombre;
